...
const PaymentStatusTable: React.FC<PaymentStatusTableProps> = ({ statuses, activeFilter }) => {
  ...
  const knownColumns = new Set(['clientName', 'expectedAmount', 'paidAmount', 'status', 'difference']);
  ...
  const onExport = () => {
    console.log("[PaymentStatusTable] Exporting reconciliation summary to CSV.");
    ...
    XLSX.writeFile(wb, 'payment_status_summary.csv');
  };

  const fullColSpan = 6 + extraColumns.length;
